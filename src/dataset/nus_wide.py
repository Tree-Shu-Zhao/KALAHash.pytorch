...
class NusWideDataset(Dataset):
    ...
    def load_data(data_dir):
        ...
        NusWideDataset.DATA = data
        NusWideDataset.CLASS_NAMES = class_names
        NusWideDataset.DATA_DIR = data_dir

    def __init__(self, split, preprocess, num_shots=None):
        super(NusWideDataset, self).__init__()
        assert NusWideDataset.DATA or NusWideDataset.CLASS_NAMES, "Call load_data first!"
        self.preprocess = preprocess

        # Split few-shot data
        if num_shots:
            assert split == "train", "We only build a few-shot data on the train split"
            cache_pth = os.path.join(NusWideDataset.DATA_DIR, f"{num_shots}shot_split_cache.pkl")
            if os.path.exists(cache_pth):
                ...
                items = split_few_shot_multi_label(NusWideDataset.DATA["train"], num_shots, 21)
                with open(cache_pth, 'wb') as f:
                    pickle.dump(items, f)
        else:
            # Return full data
            items = NusWideDataset.DATA[split]
